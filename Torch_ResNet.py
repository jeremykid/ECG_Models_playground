...
class ResNet1d(nn.Module):
    ...
    def __init__(self, input_dim, blocks_dim, n_classes, kernel_size=17, dropout_rate=0.8):
        ...
        padding = _padding(downsample, kernel_size)
        # print ('kernel_size', kernel_size, type(kernel_size))
        # 假设 kernel_size 原本可能为 int 或 tuple
        self.conv1 = nn.Conv1d(n_filters_in, n_filters_out, int(kernel_size), bias=False,
...
class ResNet1d_w_features(nn.Module):
    """Residual network for unidimensional signals.
    Parameters
    ----------
    input_dim : tuple
    ...
    def __init__(self, input_dim, blocks_dim, n_classes, kernel_size=17, dropout_rate=0.8, num_features=2):
        super(ResNet1d_w_features, self).__init__()
        # First layers
        n_filters_in, n_filters_out = input_dim[0], blocks_dim[0][0]
        n_samples_in, n_samples_out = input_dim[1], blocks_dim[0][1]
        downsample = _downsample(n_samples_in, n_samples_out)
