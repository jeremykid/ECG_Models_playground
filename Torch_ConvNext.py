...
class ConvNeXtV2_w_features(nn.Module):
    """ ConvNeXt V2 adapted for 1D data."""
    def __init__(self, in_chans=1, num_classes=1000, 
                 depths=[3, 3, 9, 3], dims=[96, 192, 384, 768], 
                 drop_path_rate=0., head_init_scale=1., return_embedding = False, num_features=0
                 ):
        super().__init__()
        self.depths = depths
        self.downsample_layers = nn.ModuleList()
        ...
        if num_features > 0:
            ...
            self.head = nn.Linear(dims[-1] + self.feature_proj_dim, num_classes)
        else:
...
def convnextv2_atto(**kwargs):
    ...
    model = ConvNeXtV2_w_features(depths=[2, 2, 6, 2], dims=[40, 80, 160, 320], **kwargs)  
    return model
...
def convnextv2_base(**kwargs):
    ...
    model = ConvNeXtV2_w_features(depths=[3, 3, 27, 3], dims=[128, 256, 512, 1024], **kwargs)    
    return model
