...
class ECGDataset(Dataset):
    ...
    def compute_weights(self):
        ...
        if len(self.diagnoses_cols) > 1:
            weights = []
            # print ('self.diagnoses_cols', self.diagnoses_cols)
    ...
    def __getitem__(self, idx):
        ...
        for lead in range(12):
            s = pd.Series(ecg_data[:, lead])
            s_linear = s.interpolate(method='linear', limit_direction='both')
            ecg_data[:, lead] = s_linear.values
        ecg_data = ecg_data.T
        ####################################################################################################################
        
        if self.pretrain or self.encode:
            # print ('self.pretrain or self.encode')
            ...
            # print ('ecg_data.shape[1] - SAMPLES_IN_5_SECONDS_AT_500HZ + 1:', ecg_data.shape[1] - SAMPLES_IN_5_SECONDS_AT_500HZ + 1)
